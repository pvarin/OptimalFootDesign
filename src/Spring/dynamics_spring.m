function [qdot] = dynamics_spring(q,params)
...
d2_delta = (L.^2.*((-7).*m.^2+(-18).*m.*M+(-8).*M.^2+3.*m.^2.*cos(2.*beta))+...
...
d2_alpha = (-2).*(L.^2.*((-7).*m.^2+(-18).*m.*M+(-8).*M.^2+3.*m.^2.*cos(2.*...
...
d2_beta = 2.*L.^(-1).*(L.^2.*((-7).*m.^2+(-18).*m.*M+(-8).*M.^2+3.*m.^2.*...
...
d_delta));
   ...
   % betaDD = 
    ...
    qdot = [d_alpha;d_beta;d_delta;d2_alpha;d2_beta;d2_delta];
